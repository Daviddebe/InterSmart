    ...
    public class DataBase
    {
        ...
        public void VullenDataBase()
        {
            ...
            foreach (var t in _tweets)
            {
                ...
                if (TweetTijd > Program.deTijd)
                {
                    ...
                    #region Tweets opslaan in database met UserID en PresentationID


                    string TweetTable = "insert into Tweet(Answer, UserID, PresentationID) values ($title, $UserId, $PresentationID)";
        ...
        public void score()
        {
            ...
            foreach (var t in _tweets)
            {
                ...
                title = title.Replace("@Inter_Smart", ""); //@Inter_Smart uit tweets verweideren
                string UpperCasetitle = title.ToUpper(); //Antwoorden in druk letters plaatsen
                UpperCasetitle.ToCharArray();

                if (Program.Antwoorden[UpperCasetitle[1]] -1 == UpperCasetitle[0])
                {
